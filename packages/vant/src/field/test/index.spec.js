...
// NEW TESTS for keyboard blocking fix with placeholder
test('should handle focus properly when placeholder exists after modal close', async () => {
  const wrapper = mount(Field, {
    props: {
  ...
  // Simulate first click - should work normally
  await input.trigger('click');
  expect(wrapper.emitted('clickInput')).toBeTruthy();
  expect(wrapper.emitted('clickInput').length).toBe(1);

  // Simulate modal close scenario by triggering blur
  await input.trigger('blur');

  // Simulate second click - should still trigger focus properly
  await input.trigger('click');
  expect(wrapper.emitted('clickInput').length).toBe(2);
});

test('should handle focus and blur events with placeholder', async () => {
  const wrapper = mount(Field, {
    props: {
      placeholder: 'Enter text',
      modelValue: '',
  ...
  // Initially no focus events
  expect(wrapper.emitted('focus')).toBeFalsy();
  expect(wrapper.emitted('blur')).toBeFalsy();

  // After clicking and focusing input
  await input.trigger('click');
  await input.trigger('focus');
...
test('should handle focus/blur cycle properly with placeholder', async () => {
  const wrapper = mount(Field, {
    props: {
      placeholder: 'Enter text',
      modelValue: '',
  ...
  // Second focus cycle (simulating the bug scenario)
  await input.trigger('click');
  await wrapper.vm.$nextTick();

  // Should be able to focus again without issues
  await input.trigger('focus');
  expect(wrapper.emitted('focus').length).toBe(2);
});

test('should emit events correctly when placeholder changes', async () => {
  const wrapper = mount(Field, {
    props: {
  ...
  // Should still work after placeholder change
  await input.trigger('click');
  expect(wrapper.emitted('clickInput').length).toBe(2);
});

test('should handle readonly state properly in focus events', async () => {
  const wrapper = mount(Field, {
    props: {
      readonly: true,
  ...
  // Focus on readonly field should trigger blur immediately
  await input.trigger('focus');
