...
test('should minHeight 100 and maxHeight 0.8 innerHeight when anchors props do not', async () => {
  const wrapper = mount({
    render() {
  ...
  expect((wrapper.element as HTMLDivElement).style.height).toBe('480px');
...
test('should only drag header when allowSlidingContent is false', async () => {
  const wrapper = mount({
    render() {
      return (
        <FloatingPanel
