/**
 * build iconfont from sketch
 */
const fs = require('fs-extra');
const gulp = require('gulp');
const path = require('path');
const glob = require('fast-glob');
const shell = require('shelljs');
const md5File = require('md5-file');
const iconfont = require('gulp-iconfont');
const iconfontCss = require('gulp-iconfont-css');
const config = require('../../packages/icon/config');
const templateLocal = require('../../packages/icon/config/template-local');

const find = dir => path.join(__dirname, dir);
const iconDir = find('../../packages/icon');
const cssDir = find('../../packages/vant-css/src');
const svgDir = path.join(iconDir, 'svg');
const sketch = path.join(iconDir, 'assert/icons.sketch');
const template = path.join(iconDir, 'config/template.css');

// extract svg from sketch
// should install sketchtool first
// install guide: https://developer.sketchapp.com/guides/sketchtool/
function extractSvg() {
  shell.exec(
    `sketchtool export slices --formats=svg --overwriting=YES --save-for-web=YES --output=${svgDir} ${sketch}`
  );
}

// get icon unicode
function getCodePoints() {
  const codePoints = {};
  config.glyphs.forEach((icon, index) => {
    const src = path.join(svgDir, icon.src);
    if (fs.existsSync(src)) {
      fs.renameSync(src, path.join(svgDir, icon.css + '.svg'));
      codePoints[icon.css] = 0xf000 + index;
    }
  });
}

// generate ttf from sketch && build icon.css
gulp.task('ttf', () => {
  extractSvg();

  const fixedCodepoints = getCodePoints();

  // remove previous ttf
  const prevTTFs = glob.sync(path.join(iconDir, '*.ttf'));
  prevTTFs.forEach(ttf => fs.removeSync(ttf));

  return gulp
    .src([`${svgDir}/*.svg`])
    .pipe(
      iconfontCss({
        fontName: config.name,
        path: template,
        targetPath: '../../packages/vant-css/src/icon.css',
        normalize: true,
        fixedCodepoints
      })
    )
    .pipe(
      iconfont({
        fontName: config.name,
        formats: ['ttf']
      })
    )
    .pipe(gulp.dest(iconDir));
});

gulp.task('default', ['ttf'], () => {
  // generate ttf hash
  const ttfPath = path.join(iconDir, 'vant-icon.ttf');
  const hash = md5File.sync(ttfPath).slice(0, 6);
  const name = `vant-icon-${hash}.ttf`;
  fs.renameSync(ttfPath, path.join(iconDir, name));

  // modify icon.css
  let source = fs.readFileSync(path.join(cssDir, 'icon.css'), 'utf-8');
  source = source.replace('vant-icon.ttf', name);
  fs.writeFileSync(path.join(cssDir, 'icon.css'), source);

  // generate icon-local.css
  fs.writeFileSync(path.join(cssDir, 'icon-local.css'), templateLocal(config.name, hash));

  // upload ttf to cdn
  shell.exec(`superman cdn /vant ${path.join(iconDir, name)}`);
});
